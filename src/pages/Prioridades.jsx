...
export default function Prioridades() {
  ...
  function remove(id) { if (!confirm("¿Eliminar prioridad?")) return; setItems(items.filter(i => i.id !== id)); }

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Administrar Prioridades</h1>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        <table className="min-w-full">
          ...
          <tbody>
            ...
            {items.length === 0 && <tr><td colSpan={3} className="text-center p-6 text-gray-500">No hay prioridades.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-4">{editing ? "Editar Prioridad" : "Nueva Prioridad"}</h3>
